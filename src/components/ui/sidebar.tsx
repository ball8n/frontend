...
import logo from '@/public/ball8n_seethrough.png'
const navItems = [
  {
    name: "Products",
    href: "/products",
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 flex h-full w-64 flex-col border-r bg-white transition-transform lg:translate-x-0",
          isMobileOpen ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="flex h-16 items-center border-b px-6">
        <Link href="/" className="flex items-center font-bold text-xl">
          <Image src={logo} alt="Ball8n" width={256} height={256} />
          </Link>
        </div>
